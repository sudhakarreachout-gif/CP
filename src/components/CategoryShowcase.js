...
const categoryData = {
  ...
  Grooming: {
    ...
    color: "bg-butter/10",
    accent: "text-[#B8860B]",
    buttonBg: "bg-butter text-[#B8860B]",
    dotBg: "bg-butter",
    products: [4, 11]
  },
  "Eco Product": {
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              <span className={`font-nunito font-bold uppercase tracking-[0.2em] text-[10px] lg:text-xs ${data.accent}`}>
                Care & Community
              </span>
              <button onClick={onClose} className="p-2 hover:bg-espresso/5 rounded-full transition-colors clickable">
                <X className="w-6 h-6 text-espresso" />
              </button>
            </div>
            
            <h2 className="text-3xl lg:text-[48px] font-serif font-bold text-espresso leading-tight">{data.title}</h2>
            <p className="text-base lg:text-lg font-jakarta text-text-muted leading-relaxed max-w-xl">
              {data.description}
            </p>
            
            <div className="grid sm:grid-cols-2 gap-8 pt-4">
              <div className="space-y-4">
                <h4 className="font-nunito font-bold text-xs uppercase tracking-widest text-text-muted flex items-center gap-2">
                  ✨ Pro Care Tips
                </h4>
                <ul className="space-y-4">
                  {data.careTips.map((tip, i) => (
                    <li key={i} className="flex items-center gap-3 text-sm font-jakarta font-medium text-espresso">
              ...
              <div className="flex items-end">
                <button className={`btn-primary w-full flex items-center justify-center gap-2 group clickable text-sm ${data.buttonBg}`}>
